...
1. *version-based* where all installables that meet the version constraints are
   recorded;
2. *file-based* where one installable is recorded per supported platform and Python version.
...
Source distributions can specify dynamic dependencies. This means it is not
necessarily possible to, ahead of time, know the actual dependencies and thus
lock them. This is a limitation caused by the freedom that build backends have,
and needs to be accepted. It is up to third-party tooling to choose how to deal
with this issue.
